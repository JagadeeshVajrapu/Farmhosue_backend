...
function luxuryEmailTemplate(data: BookingEmailData): string {
  return `
<!DOCTYPE html>
<html>
...
export async function sendAdminBookingAlert(data: BookingEmailData): Promise<boolean> {
  const transporter = createTransporter();
...
function contactEnquiryEmailTemplate(data: ContactEnquiryEmailData): string {
  ...
  const submittedDate = escapeHtml(
